...
def decoding_character(morse_character):
    ...
    for alpha, mose in morse_code_dict.items():  # for name, age in dictionary.iteritems():  (for Python 2.x)
        if mose == morse_character:
            result = alpha
    return result
    # ==================================
...
def decoding_sentence(morse_sentence):
    ...
    for chunk in sentence:
        chunk = chunk.split()
        for alpha in chunk:
            result += decoding_character(alpha)
        result += " "
    return result[:-1]
    # ==================================
